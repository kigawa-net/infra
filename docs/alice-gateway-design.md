...
| 既存HAProxy            | `10.0.0.103`           |
| kube-vip               | static Pod                |
| alice                  | Kubernetesなし、1ホスト   |

既存の重要方針として、Kubernetes API の利用者向け endpoint は VIP `10.0.0.100` を正とする。

kube-vip 自身の leader election 用 API 参照先は、VIP 喪失時の自己参照デッドロックを避けるため、既存 HAProxy `10.0.0.103` または正常な control-plane を使う。
...
```text
permit 10.0.0.0/24
permit 10.0.0.240/32
permit 10.0.0.241/32
deny any
```
...
| `:80`          | `10.0.0.240:80`    |
| `:443`         | `10.0.0.240:443`   |
| `:25565`       | `10.0.0.241:25565` |

犬山LAN `10.0.0.0/24` の kube-vip LoadBalancer 用VIPとして以下を予約する。

| Address             | 用途                    |
| ------------------- | ----------------------- |
| `10.0.0.240`     | Ingress VIP             |
| `10.0.0.241`     | Minecraft Backend VIP   |
| `10.0.0.242-249` | 将来用VIP予約           |

実環境では既存LoadBalancerがMetalLB `main-pool` で運用されているため、alice用VIPは `main-pool` に `10.0.0.240-10.0.0.249` を追加し、既存のIngress/Minecraft Serviceとは別のalice専用LoadBalancer Serviceとして割り当てる。
...
backend http_backend
    mode tcp
    option tcp-check
    server web1 10.0.0.240:80 check

backend https_backend
    mode tcp
    option tcp-check
    server web1 10.0.0.240:443 check

backend minecraft_backend
    mode tcp
    option tcp-check
    server mc1 10.0.0.241:25565 check
```
...
| `alice-01` | `10.0.0.240` | `80/tcp` | HTTP転送 |
| `alice-01` | `10.0.0.240` | `443/tcp` | HTTPS転送 |
| `alice-01` | `10.0.0.241` | `25565/tcp` | Minecraft転送 |
...
| From | To | Port |
| --- | --- | ---: |
| `alice wg0` | `10.0.0.240` | `80/tcp` |
| `alice wg0` | `10.0.0.240` | `443/tcp` |
| `alice wg0` | `10.0.0.241` | `25565/tcp` |
