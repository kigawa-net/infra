...
- **WireGuard**:
    - `wg0` インターフェース (`172.31.255.2/24`) を使用し、外部拠点と各 k8s ノードへのセキュアな通信路を提供します。
    ...
    - **ピア構成** (`172.31.255.0/24`):
        ...
        - k8s1 (`172.31.255.11`) — 直接ピア（WireGuard のみ、BGP なし）
        - k8s2 (`172.31.255.12`) — 直接ピア（WireGuard のみ、BGP なし）
    - **鍵管理**: 各ノードは初回 apply 時に `/etc/wireguard/privatekey` (600) / `publickey` (644) を自動生成。ディレクトリは 711 で非 root がパスを知れば publickey を読める構成。Alice の apply 時に各ノードへ SSH して公開鍵を自動取得するため手動操作不要。
    - **Terraform モジュール**: `hardware/modules/wireguard` で管理。`wireguard_server_public_key` 変数に Alice の公開鍵を設定すれば有効化される。
