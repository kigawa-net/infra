...
data "external" "ssh_key" {
  program = ["bash", "-c", <<-EOT
    value=$(bws -c no secret get "${var.ssh_key_bitwarden_id}" | jq -r '.value')
...
data "external" "sudo_password" {
  program = ["bash", "-c", <<-EOT
    value=$(bws -c no secret get "${var.sudo_password_bitwarden_id}" | jq -r '.value')
    jq -n --arg value "$value" '{"value": $value}'
  EOT
  ]
}
...
data "external" "inuyama_wireguard_public_key" {
  program = ["bash", "-c", <<-EOT
    value=$(bws -c no secret get "${var.inuyama_wireguard_public_key_bitwarden_id}" | jq -r '.value')
    jq -n --arg value "$value" '{"value": $value}'
  EOT
  ]
}
...
data "external" "k8s1_wireguard_public_key" {
  program = ["bash", "-c", <<-EOT
    ...
    ssh_key=$(bws -c no secret get "${var.ssh_key_bitwarden_id}" | jq -r '.value')
    tmpkey=$(mktemp)
    chmod 600 "$tmpkey"
    printf '%s\n' "$ssh_key" > "$tmpkey"
    value=$(ssh \
