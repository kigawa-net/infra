...
resource "null_resource" "wireguard" {
  ...
  provisioner "file" {
    content     = <<-SCRIPT
      ...
      # knot/knot-resolver等、他パッケージの設定ファイル変更プロンプトで
      # apt-getが対話待ちになり失敗することがある(issue #28、k8s2で発生)。
      # 既存の設定ファイルを優先して保持し、非対話で進める。
      apt_get_retry apt-get -o Dpkg::Options::=--force-confold -o Dpkg::Options::=--force-confdef install -y wireguard
