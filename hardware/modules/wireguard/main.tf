...
resource "null_resource" "wireguard" {
  ...
  provisioner "file" {
    content     = <<-SCRIPT
      ...
      # の競合しか防げない。OS自身のunattended-upgradesが/var/lib/dpkg/lock-frontend
      # を握っていると(cronで定期実行されるため、いつ動くか制御できない)、
      # flockでは防げずapt-getがexit code 100で失敗する。dpkgロックが空くまで
      # 待ってからapt-getを呼ぶことで対処する。
      wait_for_dpkg_lock() {
