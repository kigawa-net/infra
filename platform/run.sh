...
export TF_VAR_azuread_terraform_client_secret
...
# - azuread_terraform_*: Microsoft Entraアプリ登録を操作するための、Application.ReadWrite.All
#   権限を持つTerraform自動化用サービスプリンシパル(Azure Portalで一度だけ手動作成すること)
...
TF_VAR_azuread_terraform_client_secret=$(bws -c no secret get REPLACE_WITH_AZUREAD_TERRAFORM_CLIENT_SECRET_UUID | jq -r '.value // empty') || true
