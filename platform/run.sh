...
# - azuread_terraform_client_certificate: Terraform自動化用サービスプリンシパル
#   "kalender-terraform-automation"(client_id/tenant_idはvariables.tfにデフォルト値として
#   コード管理済み、機密情報ではないため)の認証用PFX証明書をBase64エンコードした文字列
